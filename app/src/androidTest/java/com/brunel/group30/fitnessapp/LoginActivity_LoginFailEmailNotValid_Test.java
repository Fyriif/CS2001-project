...
public class LoginActivity_LoginFailEmailNotValid_Test {
        ...
        ViewInteraction editText = onView(
                allOf(withId(R.id.edit_email), withText(testEmail),
                        childAtPosition(
                                ...
                                0),
                        isDisplayed()));
        editText.check(matches(withErrorText(R.string.error_enter_valid_email)));

        if (mAuth.getCurrentUser() != null) {
            fail("User " + testEmail + " has logged in, this shouldn't occur");
        }
    }
